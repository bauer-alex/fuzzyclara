...
#' @param scale scale numeric variables before distance matrix calculation? Default TRUE
#' @param ... Additional arguments passed to the main clustering algorithm and to proxy::dist for the calculation of the distance matrix
...
claraclust <- function(data, clusters = 5, metric = "euclidean", samples = 10,
                       ...
                       seed = 1234, m = 2, verbose = 1, scale = TRUE, ...) {
  ...
  class(best_solution) <- c("claraclust", class(best_solution))
