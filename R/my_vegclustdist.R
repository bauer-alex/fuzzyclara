my_vegclustdist <- function (x, mobileMemb, fixedDistToCenters = NULL, method = "NC",
                          m = 2, dnoise = NULL, eta = NULL, alpha = 0.001, iter.max = 100,
                          nstart = 1, seeds = NULL, verbose = FALSE)
{
  vegclustonedist <- function(d, mobileMemb, fixedDistToCenters = NULL,
                              method = "NC", m = 2, dnoise = NULL, eta = NULL, alpha = 0.001,
                              iter.max = 100) {
    METHODS <- c("KM", "FCM", "PCM", "NC", "HNC", "KMdd",
                 "NCdd", "HNCdd", "FCMdd", "PCMdd")
    ...
    med = rep(NA, ncol(u))
    if ((method == "PCM" || method == "PCMdd") && length(eta) !=
        (kMov + kFix))
      stop("Vector of reference distances (eta) must have a length equal to the number of clusters")
    if (method == "NC" || method == "NCdd" || method == "HNC" ||
        method == "HNCdd") {
      u = cbind(u, vector("numeric", length = n))
    }
    uPrev = matrix(0, nrow = n, ncol = ncol(u))
    if (method == "KM" || method == "PCM" || method == "NC" ||
        method == "HNC" || method == "FCM") {
      sqdist2cent = matrix(0, nrow = n, ncol = (kMov +
                                                  kFix))
      if (kFix > 0) {
    ...
    iter = 1
    while (continue) {
      if (method == "KM" || method == "PCM" || method ==
          "NC" || method == "HNC" || method == "FCM") {
        vargeom = vector("numeric", kMov)
        for (k in 1:(kMov)) {
          vargeom[k] = sum((u[, k]^m) %*% (d^2) %*% (u[,
                                                       k]^m))/(2 * sum(u[, k]^m)^2)
          for (i in 1:n) {
            sqdist2cent[i, k] = (sum((u[, k]^m) * (d[i,
            ]^2))/sum(u[, k]^m)) - vargeom[k]
            if (sqdist2cent[i, k] < 0)
              sqdist2cent[i, k] = 0
          }
        }
      }
      else {
        # NEW initialize the med vector as mobileMemb
        if (all(is.na(med))) {
          med <- mobileMemb
        }

        for (k in 1:kMov) {
          # 1. Update squared distances to mobile centers (medoids for the non Euclidean-based methods)
          #
          # Anmerkungen Alex
          # Was sind die Objekte genau?
          #  -> u: Jede Spalte gibt als Info fuer einen Cluster an, welche Beobachtung der aktuelle Medoid ist(?)
          #  -> d: Distanzmatrix, so wie uebergeben an die Funktion
          # -> TODO WAS macht die naechste Zeile inhaltlich?
          #         Fuer Cluster k gibt die Zeile doch einfach nur die eine Beobachtung aus,
          #         die die geringste Distanz zum aktuellen Clustermedoiden hat, oder?
          #         Die berechnet ja nicht den Medoiden als Punkt mit der geringsten Distanz zu ALLEN anderen
          #         Beobachtungen im Cluster.
          # browser()
          # NEW calculations without evaluating other cluster medoids as medoids for the current cluster
          medoids_otherClusters <- med[-k]
          d_withoutOtherMedoids <- d[,-medoids_otherClusters]
          med[k] = as.numeric(colnames(d_withoutOtherMedoids)[which.min((u[, k]^m) %*% d_withoutOtherMedoids)])
          dist2med[, k] = d[, med[k]]
          # OLD calculations
      ...
      if (method == "KM") {
        ...
        for (k in 1:ncol(sqdist2cent)) u[, k] = 1/(1 +
                                                     ((sqdist2cent[, k])/eta[k])^(1/(m - 1)))
        u[dist2cent == 0] = 1
      }
    ...
    if (method == "FCM" || method == "KM")
      functional = sum(sqdist2cent * (u^m))
    else if (method == "NC" || method == "HNC")
      functional = sum(sqdist2cent * (u[, -(kMov + kFix +
                                              1)]^m)) + sum(dnoise^2 * u[, kMov + kFix + 1]^m)
    else if (method == "FCMdd" || method == "KMdd")
      functional = sum(dist2med * (u^m))
    else if (method == "NCdd" || method == "HNCdd")
      functional = sum(dist2med * (u[, -(kMov + kFix +
                                           1)]^m)) + sum(dnoise * u[, kMov + kFix + 1]^m)
    else if (method == "PCM") {
      functional = 0
      for (k in 1:(kMov + kFix)) functional = functional +
          sum(sqdist2cent[, k] * (u[, k]^m)) + sum(eta[k] *
                                                     (1 - u[, k])^m)
    }
    else if (method == "PCMdd") {
      functional = 0
      for (k in 1:(kMov + kFix)) functional = functional +
          sum(dist2med[, k] * (u[, k]^m)) + sum(eta[k] *
                                                  (1 - u[, k])^m)
    }
    if (verbose)
    ...
    if (method == "NC" || method == "NCdd" || method == "HNC" ||
        method == "HNCdd")
      names(u)[kMov + kFix + 1] = "N"
    rownames(u) = rownames(d)
    rownames(dist2cent) = rownames(d)
    size = colSums(u[, 1:(kMov + kFix)])
    if (method == "NC" || method == "FCM" || method == "KM" ||
        method == "PCM" || method == "HNC")
      withinss = colSums((dist2cent^2) * (u[, 1:(kMov +
                                                   kFix)]^m))
    else withinss = colSums((dist2cent) * (u[, 1:(kMov +
                                                    kFix)]^m))
    mobileCenters = NULL
    fixedCenters = NULL
    if (method == "KMdd" || method == "FCMdd" || method ==
      ...
      if (kFix > 0)
        fixedCenters = med[(kMov + 1):(kMov + kFix)]
    }
    res = list(mode = "dist", method = method, m = m, dnoise = dnoise,
               eta = eta, memb = u, mobileCenters = mobileCenters,
               fixedCenters = fixedCenters, dist2clusters = dist2cent,
               withinss = withinss, size = size, functional = functional)
    class(res) <- "vegclust"
    return(res)
  }
  if (is.null(seeds))
    seeds = 1:nrow(as.matrix(x))
  if (is.vector(mobileMemb) && length(mobileMemb) == 1 && is.numeric(mobileMemb)) {
    bestRun = vegclustonedist(x, mobileMemb = sample(seeds,
                                                     mobileMemb), fixedDistToCenters, method, m, dnoise,
                              eta, alpha, iter.max)
    if (nstart > 1) {
      for (i in 2:nstart) {
        run = vegclustonedist(x, mobileMemb = sample(seeds,
                                                     mobileMemb), fixedDistToCenters, method, m,
                              dnoise, eta, alpha, iter.max)
    ...
    return(vegclustonedist(x, mobileMemb, fixedDistToCenters,
                           method, m, dnoise, eta, alpha, iter.max))
  }
}
