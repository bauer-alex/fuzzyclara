...
test_that("fuzzyclara_fuzzy_pam", { # use pam clustering if m = 1 or clusters = 1
  ...
  expect_identical(cc_fixed$medoids, cc_fuzzy$medoids)
...
test_that("fuzzyclara_dist", { # check other distance functions
  ...
  # check if same solution
  expect_identical(cc_fixed$medoids, cc_dist$medoids)
  expect_identical(cc_fixed$distance_to_medoids, cc_dist$distance_to_medoids)
...
test_that("fuzzyclara_scale", { # scaling of variables
  ...
  dist_matrix <- proxy::dist(data[cc_fixed$subsample_ids,])
