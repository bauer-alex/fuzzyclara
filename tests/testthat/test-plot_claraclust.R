test_that("plot_claraclust", { # plots
  ...
  ## grouped plot
  p <- plot(x = cc_fixed, data = USArrests_enriched, variable = "Assault", group_by = "Area", type = "grouped")
  # check class of object 
  expect_s3_class(p, "ggplot")
  expect_s3_class(p$layers[[1]]$geom, "GeomBoxplot")
  
  expect_error(plot(x = cc_fixed, data = USArrests_enriched, 
                    variable = "Assault", type = "grouped"))
  expect_error(plot(x = cc_fixed, data = USArrests_enriched, 
                    variable = "Assault", group_by = "area", type = "grouped"))
